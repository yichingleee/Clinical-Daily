...
const AppContent = () => {
  ...
  // Fetch saved article IDs when user changes
  useEffect(() => {
    const fetchSavedIds = async () => {
      if (user) {
        const ids = await getSavedArticleIds(user.id);
  ...
  const handleSaveArticle = useCallback(async (article: Article) => {
    ...
    try {
      ...
      setSavedArticleIds(prev => new Set([...prev, article.pmid]));
    } catch (error) {
      console.error('Error saving article:', error);
    }
  }, [user]);

  const handleUnsaveArticle = useCallback(async (articlePmid: string) => {
    if (!user) return;
    try {
      await unsaveArticle(user.id, articlePmid);
      setSavedArticleIds(prev => {
        const newSet = new Set(prev);
        newSet.delete(articlePmid);
        return newSet;
      });
    } catch (error) {
      ...
      <header className="sticky top-0 z-40 w-full border-b border-white/10 bg-panel/80 backdrop-blur-xl shadow-[0_10px_60px_rgba(0,0,0,0.45)]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            {/* Logo */}
            <div className="relative flex items-center gap-3 cursor-pointer" onClick={() => setCurrentPage('feed')}>
            ...
            <div className="flex items-center gap-3">
              {/* Library link for logged-in users */}
              {user && currentPage === 'feed' && (
                <button
                  onClick={() => setCurrentPage('library')}
                  className="hidden md:flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-medium text-slate-200 hover:border-accent-500/40 hover:text-accent-300 transition-all"
                >
        ...
        <div className="flex flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
          <div className="relative w-full rounded-3xl border border-white/5 bg-white/5 backdrop-blur-xl shadow-[0_25px_80px_rgba(0,0,0,0.45)]">
            ...
            <div className="p-6 sm:p-8">
              {currentPage === 'library' ? (
                <LibraryPage onBack={() => setCurrentPage('feed')} />
              ) : (
                      ...
                      {isRefreshing && articles.length === 0 ? (
                        ...
                        processedArticles.map(article => (
                          <ArticleCard
                            ...
                            isSaved={savedArticleIds.has(article.pmid)}
                            showSaveButton={true}
                          />
                        ))
                      ) : (
